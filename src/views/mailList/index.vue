...
<script>
import {
  JDGetPeasonalContactInfo,
  JDGetPeasonalContactLetter
} from "@/api/getData";
...
export default {
  ...
  methods: {
    ...
    close() {},
    init() {},
    async GetPeasonalContactInfo() {
      //2.获取通讯录信息
      let data = await JDGetPeasonalContactInfo({
      ...
      //1.获取通讯录信息字母列表
      let letterA = await JDGetPeasonalContactLetter({
				//KeyWord: this.keyWord,
				//Flag:"1"
			});
